...
# 05 Best Open-Source Case Management Software for Your Law Firm
def test_blog1(driver):
    wait = WebDriverWait(driver,10)
    blog = wait.until(EC.element_to_be_clickable((By.XPATH,"/html/body/div[1]/main/ul/li[1]/a")))
    blog.click()
...
def test_links(driver):
    ...
    # go to clinic classes
    clinic_classes = wait.until(EC.element_to_be_clickable((By.XPATH,"//a[normalize-space()='ClinicCases']")))
    clinic_classes.click()
    print(driver.title)
    driver.back()

    # go to Ark case
    arkcase = wait.until(EC.element_to_be_clickable((By.XPATH,"//a[normalize-space()='Arkcase']")))
    arkcase.click()
    print(driver.title)
    driver.back()

    # go to Case fox
    case_fox = wait.until(EC.element_to_be_clickable((By.XPATH,"//a[normalize-space()='CaseFox']")))
    case_fox.click()
    print(driver.title)
    driver.back()

    # go to Dib case
    dib_case = wait.until(EC.element_to_be_clickable((By.XPATH,"//a[normalize-space()='Dibcase']")))
    dib_case.click()
    print(driver.title)
    driver.back()

    # go to MylegalNET
    mylegalnet = wait.until(EC.element_to_be_clickable((By.XPATH,"//a[normalize-space()='MyLegalNet']")))
    mylegalnet.click()
    print(driver.title)
    driver.back()
